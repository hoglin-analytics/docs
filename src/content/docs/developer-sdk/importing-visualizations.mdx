...
great way to show off the possibilities of your Hoglin integration. Our SDK provides simple ways to achieve this. For
...
When talking about an exported visualization, we refer to it as a "snapshot". In essence, a snapshot is
a static copy of a visualization at a specific point in time. When a visualization is exported from the Hoglin
dashboard, its current state is taken and stored. Snapshots are not tied to the original visualization. If the original
visualization is updated or deleted later, the snapshot remains unchanged, and importing it again will yield the same
result.
...
already exists on the dashboard. Specifying this parameter with `Hoglin#importSnapshot` is recommended as it does this logic in a
round trip to the Hoglin backend, unlike manually checking with `Hoglin#isVisualizationImported()`, which would require two
separate requests.
